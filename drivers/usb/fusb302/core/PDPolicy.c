...
volatile       FSC_U32                  PolicyStateTimer;                               
volatile       FSC_U32                  NoResponseTimer;                                
volatile static FSC_U32                  SwapSourceStartTimer;                           
...
void PolicySourceStartup(void)
{
    ...
    switch (PolicySubIndex)
    {
        case 0:
            ...
            CapsCounter = 0;                                                    
            CollisionCounter = 0;                                               
            PolicyStateTimer = 150;                                            
            PolicySubIndex++;
            break;
        case 1:
            if ((isVBUSOverVoltage(VBUS_MDAC_4P62) && (SwapSourceStartTimer == 0)) || (PolicyStateTimer == 0))                     
            {
                PolicySubIndex++;
            }
            break;
        ...
        switch (PolicySubIndex)
        {
            case 0:
                if (PolicySendData(DMTSourceCapabilities, CapsHeaderSource.NumDataObjects, &CapsSource[0], peSourceSendCaps, 1, SOP_TYPE_SOP) == STAT_SUCCESS)
                {
                    ...
                    CapsCounter = 0;                                            
                    NoResponseTimer = T_TIMER_DISABLE;                          
                    PolicyStateTimer = tSenderResponse - tHardResetOverhead;                     
                }
                break;
            default:
                if (ProtocolMsgRx)                                              
...
void PolicySourceTransitionSupply(void)
{
    ...
    switch (PolicySubIndex)
    {
        ...
        case 5:
            if(CapsSource[USBPDContract.FVRDO.ObjectPosition-1].FPDOSupply.Voltage == 100)
            {
                if((!isVBUSOverVoltage(VBUS_MDAC_5P04) && isVBUSOverVoltage(VBUS_MDAC_4P62)) || (PolicyStateTimer == 0))   
                {
                    PolicySubIndex++;
                }
            }
            ...
            {
                if((isVBUSOverVoltage(VBUS_MDAC_11P76))  || (PolicyStateTimer == 0))                           
                {
                    PolicySubIndex++;
                }
            }
            ...
            break;
        default:
            if (PolicySendCommand(CMTPS_RDY, peSourceReady, 0) == STAT_SUCCESS)                     
            {
...
void PolicySinkEvaluatePRSwap(void)
{
    ...
    switch(PolicySubIndex)
    {
        ...
        default:    
            ...
            {
                Status = PolicySendCommandNoReset(CMTPS_RDY, peSourceStartup, 0);   
                if (Status == STAT_ERROR) PolicyState = peErrorRecovery;        
                SwapSourceStartTimer = tSwapSourceStart;
            }
            break;
    }
...
void processDMTBIST(void)
{
    ...
    switch (bdo)
    {
        ...
        if(Registers.DeviceID.VERSION_ID == VERSION_302B)
        {
                Registers.Control.BIST_TMODE = 1;                               
                DeviceWrite(regControl3, 1, &Registers.Control.byte[3]);
        }
        else
        {
