...
FSC_U8			 loopCounter = 0;		 
static USBTypeCCurrent	toggleCurrent;			
USBTypeCCurrent  SourceCurrent;					
...
extern bool flag_DualRoleSet_DFP;
...
extern void core_enable_typec(FSC_BOOL enable);
...
static bool is_RaRaCable = false;
void SendRaRaEvent(bool value)
{
	printk("FUSB  [%s] - Send Uevent to notify framework that a Ra-Ra cable is %s\n", __func__, value?"connected":"disconnected");
...
void StateMachineTypeC(void)
{
	...
	if (fusb_debug_level >= 0)
		printk("FUSB  [%s]: leave StateMachineTypeC(),(R:%d/W:%d) now ConnState:%d(%d circles), CC1TermPrevious:%d, CC2TermPrevious:%d, "
			"flag_DualRoleSet:%d, PortType:%d, blnSrcPreferred:%d, blnSnkPreferred:%d, flag_DualRoleSet_DFP:%d, loopCounter:%d\n",
			__func__, readI2C_failcount, writeI2C_failcount, ConnState, state_runcount, CC1TermPrevious, CC2TermPrevious,
			flag_DualRoleSet, PortType, blnSrcPreferred, blnSnkPreferred, flag_DualRoleSet_DFP, loopCounter);
...
void StateMachineAttachWaitAccessory(void)
{
	...
	else if (((CC1TermPrevious == CCTypeOpen) && blnCCPinIsCC1) || ((CC2TermPrevious == CCTypeOpen) && blnCCPinIsCC2))		
	{
		SetStateDelayUnattached();
	}
...
void StateMachineAttachedSource(void)
{
    ...
    {
        case 0:
		if (flag_DualRoleSet_DFP && StateTimer != 0) {
			...
			if((StateTimer == 0) && (loopCounter != 0))													
			{
				...
				if((PolicyState == peSourceReady) || (USBPDEnabled == FALSE))
				{
					...
					if (flag_DualRoleSet_DFP)
						flag_DualRoleSet_DFP = FALSE;
				}
#endif	
			}
...
void StateMachineTrySource(void)
{
	...
	else if ((CC1TermPDDebounce == CCTypeOpen) && (CC2TermPDDebounce == CCTypeOpen))  
	{
		SetStateTryWaitSink();													 
	}
}
...
void stateMachineTryWaitSource(void)
{
	...
	if(StateTimer == 0)
	{
		if (!(((CC1TermPrevious >= CCTypeRdUSB) && (CC1TermPrevious < CCTypeUndefined))
				&& ((CC2TermPrevious == CCTypeRa) || CC2TermPrevious == CCTypeOpen))	
				&& !(((CC2TermPrevious >= CCTypeRdUSB) && (CC2TermPrevious < CCTypeUndefined))
				&& ((CC1TermPrevious == CCTypeRa) || CC1TermPrevious == CCTypeOpen))) 
		{
			SetStateDelayUnattached();
		}
	}
...
void SetStateUnattached(void)
{
	...
	if (!flag_DualRoleSet && flag_DualRoleSet_DFP)
		flag_DualRoleSet_DFP = FALSE;
	if (is_RaRaCable) {
		is_RaRaCable = false;
		SendRaRaEvent(is_RaRaCable);
	...
	if (Registers.Control.HOST_CUR != toggleCurrent) 
	{
...
void SetStateAttachWaitSource(void)
{
    ...
    else
    {
		...
		if(Registers.DeviceID.VERSION_ID == VERSION_302B)
		{
			Registers.Switches.byte[0] = 0xC8;							 
		}
		else
		{
...
void updateSourceMDACHigh(void)
{
	switch(Registers.Control.HOST_CUR)
    {
		...
		case 0b10:
            ...
            break;
		case 0b11:
			Registers.Measure.MDAC = MDAC_2P604V;					  
            break;
        default:                                                    
			Registers.Measure.MDAC = MDAC_1P596V;					  
            break;
...
void updateSourceMDACLow(void)
{
    ...
    {
		case 0b01:
			Registers.Measure.MDAC = MDAC_0P210V;					  
            break;
		case 0b10:
			Registers.Measure.MDAC = MDAC_0P420V;					  
            break;
		case 0b11:
			Registers.Measure.MDAC = MDAC_0P798V;					  
            break;
        default:                                                    
			Registers.Measure.MDAC = MDAC_1P596V;					  
            break;
...
void checkForAccessory(void)
{
	...
	if((CC1TermPrevious > CCTypeOpen) && (CC1TermPrevious < CCTypeUndefined) && (CC2TermPrevious > CCTypeOpen) && (CC2TermPrevious < CCTypeUndefined))	  
	{
		SetStateAttachWaitAccessory();
	}
	else																	
