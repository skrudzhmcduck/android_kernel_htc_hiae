...
#define cHSML_UUID_SIZE             18 /*++ 2015/08/21, USB Team, PCN00075 ++*/

#define cHSML_12_CAP_ENDIAN         (1 << HSML_12_CAP_ENDIAN) /*++ 2015/08/21, USB Team, PCN00075 ++*/
...
static void projector2_send_fb(struct projector2_dev *dev)
{
	...
	int last_pkt = 0; /*++ 2015/08/21, USB Team, PCN00074 ++*/
	...
	while (count > 0 || last_pkt == 1) { /*++ 2015/08/21, USB Team, PCN00074 ++*/
		req = projector2_req_get(dev, &dev->tx_idle);
		if (req) {
/*++ 2015/08/21, USB Team, PCN00074 ++*/
			if (last_pkt == 1) {
				last_pkt = 0;
				req->length = 0;
...
static uint projector2_send_fb2(struct projector2_dev *dev) /*++ 2015/08/21, USB Team, PCN00075 ++*/
{
	struct usb_request *req;
	int xfer;
	...
	int count = dev->hsml_proto->set_display_info.wHeight *
		dev->hsml_proto->set_display_info.wWidth * (dev->bitsPixel / 8);
	uint uXferCnt = 0; /*++ 2015/08/21, USB Team, PCN00075 ++*/
...
/*-- 2015/08/21, USB Team, PCN00075 --*/
	...
	uXferCnt ++; /*++ 2015/08/21, USB Team, PCN00075 ++*/
	while ((count > 0 || last_pkt == 1) && dev->online) { /*++ 2015/08/21, USB Team, PCN00074 ++*/
		while (!(req = projector2_req_get(dev, &dev->tx_idle))) {
			msleep(1);
...
unlock:
	if (!dev->hsml_proto->debug_mode)
		minifb_unlockbuf();
	return uXferCnt; /*++ 2015/08/21, USB Team, PCN00075 ++*/
}
...
/*-- 2015/08/21, USB Team, PCN00075 --*/
                ...
                case HSML_12_REQ_STOP_FB_TRANS:
                    projector2_enable_fb_work(prj2_dev, cHSML_STREAM_OFF); /*++ 2015/08/21, USB Team, PCN00075 ++*/
                    value = 0;
                    break;
...
static int projector2_bind_config(struct usb_configuration *c)
{
	...
	/*atomic_set(&prj2_dev->prj2_enable_HSML, 1);*/ /*++ 2015/08/20 USB Team, PCN00072 ++*/
