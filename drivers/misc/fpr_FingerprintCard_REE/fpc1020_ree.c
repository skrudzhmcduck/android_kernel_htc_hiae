...
static int device_prepare(struct fpc1020_data *fpc1020, bool enable)
{
	...
	if (enable && !fpc1020->prepared) {
		...
		rc = regulator_enable(fpc1020->regulator_l22);
		if (rc) {
            dev_err(dev, "%s(%d) Fail to enable l22 regulator\n", __func__, __LINE__);
		}else{
...
static int fpc1020_probe(struct spi_device *spi)
{
	...
	if (IS_ERR(fpc1020->regulator_l22))
	{
		...
		rc = PTR_ERR(fpc1020->regulator_l22);
		return rc;
	}else
	{
		dev_info(dev, "Get L22 Regulator Source\n");
	}
